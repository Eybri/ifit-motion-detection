...
import { getToken, getUserId, getUserInfo } from "../../utils/auth"; // Import getUserInfo
...
const HistoryResults = () => {
  ...
  // Fetch user results and user info
  const fetchUserResults = async () => {
    try {
      ...
      const response = await axios.get("http://localhost:5000/api/leaderboard", {
        params: { fetch_all: true, user_id: userId },
  ...
  // Generate PDF with user info and performance analysis
  const generateAnalysisPDF = (averages) => {
    ...
    // Logo paths - adjust these to your actual image locations
    const logoLeftPath = "/images/tup.png";
    const logoRightPath = "/images/1.png";

    // Create image elements to load the logos
    const loadImages = () => {
      return new Promise((resolve) => {
        ...
        const checkAllLoaded = () => {
          loadedCount++;
          if (loadedCount === 2) resolve({ imgLeft, imgRight });
        };

        imgLeft.onload = checkAllLoaded;
        imgRight.onload = checkAllLoaded;

        // Handle errors
        imgLeft.onerror = () => {
          console.error("Failed to load left logo");
          imgLeft.src = ""; // Set to empty to prevent further errors
          checkAllLoaded();
        };

        imgRight.onerror = () => {
          console.error("Failed to load right logo");
          imgRight.src = ""; // Set to empty to prevent further errors
          checkAllLoaded();
        };

        // Set source to load the images
        imgLeft.src = logoLeftPath;
        imgRight.src = logoRightPath;
      });
    };

    // Load images and then generate the PDF
    loadImages().then(({ imgLeft, imgRight }) => {
      // Add left logo (TUP) if loaded
      if (imgLeft.src) {
        try {
          doc.addImage(imgLeft, "PNG", 15, 10, 30, 15);
        } catch (error) {
          console.error("Error adding left logo to PDF:", error);
        }
      }

      // Add right logo (1.png) if loaded
      if (imgRight.src) {
        try {
          doc.addImage(imgRight, "PNG", 165, 10, 30, 15);
        } catch (error) {
...
// Add left logo (tup.png)
doc.addImage("images/tup.png", "PNG", 10, 5, 20, 20); // Adjust the x, y, width, and height as needed

// Add right logo (1.png)
doc.addImage("images/1.png", "PNG", 180, 5, 20, 20); // Adjust the x, y, width, and height as needed
      ...
      doc.setFontSize(12);
      doc.setFont("helvetica", "bold");
      doc.text("USER INFORMATION", 105, 55, { align: "center" });

      doc.setFont("helvetica", "normal");
      if (userInfo) {
        doc.text(`Name: ${userInfo.name}`, 20, 65);
        doc.text(`Email: ${userInfo.email}`, 20, 75);
        doc.text(`User ID: ${getUserId()}`, 20, 85);
      } else {
        doc.text("User information not available.", 20, 65);
      }
      ...
      doc.setFont("helvetica", "bold");
      doc.text("PERFORMANCE SUMMARY", 105, 105, { align: "center" });
      doc.setFont("helvetica", "normal");

      // Add performance metrics with improved formatting
      doc.text(`Performance Score: ${averages.performance_score}`, 25, 115);
      doc.text(`Motion Matching: ${averages.motion_matching_score}`, 25, 125);
      doc.text(`Energy Expenditure: ${averages.energy_expenditure} calories`, 25, 135);
      doc.text(`Steps Per Minute: ${averages.steps_per_minute}`, 120, 115);

      // Add recommendations section title
      doc.setFont("helvetica", "bold");
      doc.text("DETAILED RECOMMENDATIONS", 105, 150, { align: "center" });
  ...
  return (
    <Box
      sx={{
        ...
        "&:hover": { transform: "translateY(-4px)", boxShadow: "0px 12px 32px rgba(0, 0, 0, 0.7)" },
      }}
    >
      ...
      >
        ...
        {averages ? (
          <Button
            ...
            onClick={() => {
              if (userInfo) {
                generateAnalysisPDF(averages);
              } else {
                toast.error("User information not available.", { position: "bottom-right" });
              }
            }}
          >
